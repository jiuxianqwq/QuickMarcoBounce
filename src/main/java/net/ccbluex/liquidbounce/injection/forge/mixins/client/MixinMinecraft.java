...
public abstract class MixinMinecraft {
    ...
    private void startGame(CallbackInfo callbackInfo) throws ExecutionException, InterruptedException {
        try {
            ViaMCP.create();
            ViaMCP.INSTANCE.initAsyncSlider(5, 5, 90, 20);
        } catch (Throwable e) {
    ...
    public void sendClickBlockToController(boolean leftClick, CallbackInfo ci) {
        ...
        if (leftClickCounter <= 0 && (!thePlayer.isUsingItem() || LiquidBounce.INSTANCE.getModuleManager().getModule(MultiActions.class).getState())) {
            ...
            } else if (!LiquidBounce.INSTANCE.getModuleManager().getModule(AbortBreaking.class).getState()) {
